...
class WP_DLM {
	...
	public function plugin_links( $links ) {
		$plugin_links = array(
			'<a href="' . admin_url( 'edit.php?post_type=pirenko_portfolios&page=download-monitor-settings' ) . '">' . __( 'Settings', 'download_monitor' ) . '</a>',
	...
	public function register_globals() {
		$GLOBALS['pirenko_portfolios'] = null;
	}
	...
	public function setup_download_data( $post ) {
		...
		$GLOBALS['pirenko_portfolios'] = new DLM_Download( $post->ID );
	}
	...
	public function init_taxonomy() {
		...
		if ( post_type_exists( "pirenko_portfolios" ) )
			return;
		/**
		 * Taxonomies
		 */
		register_taxonomy( 'pirenko_portfolios_category',
	        array( 'pirenko_portfolios', 'pirenko_portfolios_version' ),
	        array(
	            'hierarchical' 			=> true,
		...
		register_taxonomy( 'pirenko_portfolios_tag',
	        array( 'pirenko_portfolios', 'pirenko_portfolios_version' ),
	        array(
	            'hierarchical' 			=> false,
		 ...
		 * Post Types
		 */
		register_post_type( "pirenko_portfolios",
			apply_filters( 'dlm_cpt_pirenko_portfolios_args', array(
				'labels' => array(
				...
				'rewrite' 				=> false,
				'query_var' 			=> true,
				'supports' 				=> apply_filters( 'dlm_cpt_pirenko_portfolios_supports', array( 'title', 'editor', 'excerpt', 'thumbnail', 'custom-fields' ) ),
				'has_archive' 			=> false,
				'show_in_nav_menus' 	=> true
			) )
		);

		register_post_type( "pirenko_portfolios_version",
			apply_filters( 'dlm_cpt_pirenko_portfolios_version_args', array(
				'public' 				=> true,
				'show_ui' 				=> true,
